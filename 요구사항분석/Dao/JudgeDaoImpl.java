...
public class JudgeDaoImpl extends Dao implements JudgeDao{
	...
	public judgeList retrieve() {
		...
		while(resultset.next()) {
			...
			judge.setJudgeID(resultset.getInt("judgeID"));
			judgeList.add(judge);
		}
		return judgeList;
		}catch (Exception e) {
	...
	public void update(String column, String updateQuery, Judge judge) {
		int judgeID = judge.getJudgeID();
		String query = "UPDATE Judge SET "+column+" = '"+updateQuery+"' WHERE JudgeID = '"+judgeID+"'";
		try {
			this.update(query);
		}catch (Exception e) {
			e.printStackTrace();
	...
	@Override
	public judgeList retrieveByResp(String resp) {
		String query = "select * from Judge WHERE responsibility = '"+resp+"'";
		judgeList judgeList = new JudgeListImpl();
		try {
		ResultSet resultset =this.retrieve(query);
		while(resultset.next()) {
